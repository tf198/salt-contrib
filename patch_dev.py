...
def get_files(target):
    ...
    for dirname, dirnames, filenames in os.walk(current_dir):
        ...
        def f(x):
            if x == '__init__.py': return False
            if x[-3:] == '.py': return True
            if parts[-1] in ('files', 'mockbin'): return True
            return False
        
        for module in filter(f, filenames):
            yield (os.path.join(dirname, module), os.path.join(target, base, rel, module))
